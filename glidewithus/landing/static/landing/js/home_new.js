...
var CogzidelHomePage = {
    ...
    opts : {},
    defaultSearchValue : 'Where are you going?',

    loadImmediatelyAdded : false,

    onBefore : function(curr, next, opts) {
        // make sure we don't call addSlide before it is defined 
        if (!opts.addSlide || !loadImmediately || loadImmediately.length === 0){
            return; 
        }
         
        /* figure out how many slides to load */
        var slidesToAdd = 10;
        var itemsToLoad = loadImmediately.length;

        if(slidesToAdd > itemsToLoad){
            slidesToAdd = itemsToLoad;
        }

        /* add appropriate slides  */
        for(var i = 0; i < slidesToAdd; i++){
            var h = loadImmediately.shift();

            var reviewHtml = '';
            if(h.reviews && h.reviews > 0){
                var reviewWord = h.reviews == 1 ? Translations.review : Translations.reviews;
                reviewHtml = ["<span class='ss_review'>", h.reviews, " ", reviewWord, "</span>"].join('');
            }

            var newSlideshowEl = 
                ["<div class='slideshow_item'>",
                    "<a href='", h.url ,"' class='image_link rounded_top_new'>",
                        "<img src='", h.picUrl, "' width='476' height='316' />",
                    "</a>",

                    "<div class='slideshow_item_details rounded_bottom'>",
                        "<img src='", h.userPicUrl, "' width='68' height='68' alt='' />",

                        "<div class='slideshow_item_details_text rounded_more'>",
                            "<div class='ss_details_top'>",
                                "<a class='ss_name' href='", h.url,"'>", h.name,"</a> - <span class='ss_location'>", h.smartLocation, "</span>",
                            "</div>",
                            "<div class='ss_details_bottom'>",
                                "<span class='ss_price'>", h.price ," / ", Translations.night, "</span>",
                                reviewHtml,
                            "</div>",
                        "</div>",
                    "</div>",
            ...
            opts.addSlide(newSlideshowEl); 
        }
    },

    init : function(opts){
        CogzidelHomePage.opts = opts || {};
        ...
        $('#ss_button_pause_play').click(function(el){
            if($('#ss_button_pause_play').hasClass('ss_button_pause')){
                $('#ss_button_pause_play').removeClass('ss_button_pause');
                $('#ss_button_pause_play').addClass('ss_button_play');

                $('#slideshow_container').cycle('pause');
            } 
            else {
                $('#ss_button_pause_play').removeClass('ss_button_play');
                $('#ss_button_pause_play').addClass('ss_button_pause');
        ...
        $('#selBanner').hover(
            function(){ $('#slideshow_controls').show(); },
            function(e){ 
                //targetEl is the element where your mouse moved into
                var targetEl = $(e.relatedTarget);

                if($.inArray('slideshow', targetEl.parents()) === -1 ){
                    $('#slideshow_controls').hide(); 
                }
            }
            //function() { log('hoverout'); }
        );
    ...
    initLocationBar : function(){
        $.each($('.inner_text'), function(index, e){
            ...
            CogzidelHomePage.fields_to_clear_on_submit.push(inputField);
        ...
        if(CogzidelHomePage.opts.location !== undefined){
            $('#location').val(CogzidelHomePage.opts.location);
    ...
    initCalendars: function() {
		...
		try {
			$.datepicker.parseDate(dateFormat, $checkin.val());
			$.datepicker.parseDate(dateFormat, $checkout.val());
		} catch(e) {
			$checkin.val(dateFormat);
			$checkout.val(dateFormat);
			jQuery('#search_inputs').css("background-color", '#ffe75f');
		}

		jQuery('#search_form').cogzidelInputDateSpan({defaultsCheckin: checkinCalendarOptions,
    ...
    locationIsBlank : function() { 
        return (!$('#location').val() || (CogzidelHomePage.defaultSearchValue ==$('#location').val())); 
    },

	checkInputs : function() {
		if (CogzidelHomePage.locationIsBlank() === true) { 
...
}; //end var CogzidelHomePage

var HomePageSlideshow = {

    keypressListenerEnabled : false,
    freezeSlideshowControls : false, //set to true to rate limit when holding down left and right arrows

    deactivateSlideshowControls : function(){
        setTimeout(function(){ HomePageSlideshow.freezeSlideshowControls = false; }, 200);
    }, 
    ...
    enableKeypressListener : function(){
        if(HomePageSlideshow.keypressListenerEnabled === false){ //only init once


            //different browsers like different events
            var keyEventName = $.browser.mozilla ? 'keypress' : 'keydown';


            $(document.documentElement).bind( keyEventName, function(e) {
            ...
            HomePageSlideshow.stopPropagationOnInputs(keyEventName);

            HomePageSlideshow.keypressListenerEnabled = true;
        }
    }, 

    //stop propagation of the keypress or keydown event when using arrows in an input field (like location bar) so the slideshow does not go left & right
    //keyEventName accepts either keypress or keydown
    stopPropagationOnInputs : function(keyEventName){

        $('input').bind( keyEventName, function(e){
            var code = e.keyCode || e.which;

            if(code == $.ui.keyCode.LEFT || code == $.ui.keyCode.RIGHT){
                e.stopPropagation();
            }
        });
    }
...
/*
 *
 * To get more effects, need to include contents from jquery.cycle.all.min.js
 *
 *
